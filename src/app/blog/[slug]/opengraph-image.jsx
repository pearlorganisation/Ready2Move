...
export default async function Image({ params }) {
    ...
    try {
        const res = await fetch(`https://api.ready2move.co.in/api/v1/blogs/${slug}`, {
        ...
        if (!blog) {
            console.error(`[OG IMAGE GEN SIMPLIFIED 1] No blog data (post.data) for ${slug}.`);
            throw new Error('blog data missing.');
        }
        console.log(`[OG IMAGE GEN SIMPLIFIED 1] blog data for ${slug}:`, JSON.stringify(blog, null, 2));


        const imageUrl = blog?.thumbImage?.secure_url;
        ...
        if (!imageUrl) {
            ...
            return new ImageResponse(
                <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'blue', color: 'white', fontSize: 30 }}>
                    Image URL not found for {blog?.title || 'blog'}.
                </div>,
                { ...size }
            );
        }
