...
export const getAllLeads = createAsyncThunk(
  "leads/getAllLeads",
  async ({ page, limit, status, propertyOrProject }: { page: number; limit: number; status?: string; propertyOrProject?: string }, { rejectWithValue }) => {
    try {
...
export const GetUserByRoles=createAsyncThunk("allLeads/getUserByRoles",async({page,limit,roles}:{page:number,limit:number, roles: string },{rejectWithValue})=>{
  ...
  try{
    ...
    const {data}=await axiosInstance.get(`/api/v1/users?page=${page}&limit=${limit}&roles=${roles}`,config)
...
export const updateLeadById = createAsyncThunk(
  ...
  async ({ id, updatedData }: { id: string; updatedData: any }, { rejectWithValue }) => {
    try {
      ...
      const { data } = await axiosInstance.patch(`/api/v1/leads/${id}`, updatedData, config);
      return data;
    } catch (error: any) {
